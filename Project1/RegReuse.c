    ...
    /*void dgemm2(double* a, double* b, double* c, int n)
    {
        ...
        for (i = 0; i < n; i+=2)
            for (j = 0; j < n; j+=2)
                 for (k = 0; k < n; k+=2)
                 {
                    register double a00 = a[(i+1)*n + k+1];
                    register double b00 = b[k*n + j];


                    c[i*n + j]         = a[i*n + k]*b00 + a[i*n + k+1]*b[(k+1)*n + j]
                                         + c[i*n + j];

                    c[(i+1)*n + j]     = a[(i+1)*n + k]*b00 + a[(i+1)*n + k+1]*b[(k+1)*n + j]
                                         + c[(i+1)*n + j];

                    c[i*n + (j+1)]     = a[i*n + k]*b[k*n + (j+1)] + a00*b[(k+1)*n + (j+1)]
                                         + c[i*n + (j+1)];

                    c[(i+1)*n + (j+1)] = a[(i+1)*n + k]*b[k*n + (j+1)] + a00*b[(k+1)*n + (j+1)]
                                         + c[(i+1)*n + (j+1)];
...
int main()
{
    ...
    printf("********** REGISTER REUSE: PART #1 & PART #2 ********* \n");
    ...
    for(k=0; k<6; k++)
    {
        ...
        gflops = ((double)2*pow(n[k],3) /(double)(cpu_time_used * pow(10,9)));
        //printf("MATRIX SIZE = %d \n", n[k]);
        printf("\n Returned from dgemm1() --- Exec. time = %.16f \n", cpu_time_used / CLOCKS_PER_SEC);
        printf("\n Returned from dgemm1() --- Gflops = %.16f \n", gflops);

        /*printf("dgemm1 PRINTING ANSWER MATRIX \n");
        ...
        /*printf("dgemm2 PRINTING ANSWER MATRIX \n");
        ...
        printf("Error for dgemm0 and dgemm1 = %f \n", error);
        if(error < 0.0001)
            printf("Error < 1e-3 \n");
        ...
        //for(i=0; i<n[k]*n[k]; i++)
          //     error = maximum(abs(c2[i]-c3[i]), error);

        printf("Error for dgemm1 and dgemm2 = %f \n", error);
        if(error < 0.0001)
            printf("Error < 1e-3 \n");
